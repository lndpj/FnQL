...
The binary-observed retail client cvar inventory, exceptional ownership cases,
behavioral parity decisions, and non-regression gates are recorded in
[`CLIENT_CVAR_COMPATIBILITY.md`](./CLIENT_CVAR_COMPATIBILITY.md).
The binary-observed retail server cvar inventory, engine/module ownership,
lifecycle and entity contracts, provider boundaries, and non-regression gates
are recorded in
[`SERVER_CVAR_COMPATIBILITY.md`](./SERVER_CVAR_COMPATIBILITY.md).
...
The retail renderer/video cvar surface, alias-ownership rules, backend
capability fallbacks, and non-regression contract are recorded in
[`RENDERER_CVAR_COMPATIBILITY.md`](./RENDERER_CVAR_COMPATIBILITY.md).
...
Its source is limited to `code/client/webui/`; `scripts/build_webpak.py`
explicitly rejects retail `bundle.js` and `web.pak` content. Release layout
validation requires both sidecar archives beside the executable.

Archive generation does not trust metadata preserved by CI artifact transfer.
The packager sorts members, normalizes owner/group/timestamps and regular-file
modes, restores mode `0755` to ELF files, and uses mode `0644` for data and
documentation. Linux archive validation reopens the tarball and requires the
`fnql` client, `fnql.ded`, all three public renderer modules, both package
sidecars, `LICENSE`, and `docs/fnql/TECHNICAL.md`; it also rejects links,
non-regular entries, unsafe paths, non-i386 ELF files, and non-canonical modes.
...
`linux-x86`; missing or additional artifact directories fail packaging.
macOS release jobs are absent because retail Quake Live has no macOS game
...
- every FnQL build, test, runtime proof, and release artifact is 32-bit x86 on
  ...
  servers match retail `qagamei386.so`. 64-bit, ARM, and macOS targets are not
  built, tested, packaged, or used as validation
...
- Linux artifacts are published as `.tar.gz`, not ZIP, so executable modes
  ...
  resulting tarball as well as staged directories, `.fnz`, and Windows ZIPs.

Renderer-focused verification lives beside the release packaging flow:

- [`docs/fnql/RTX_RENDERER.md`](./RTX_RENDERER.md) defines the optional fifth
  renderer's Quake Live API/BSP boundary, hybrid base/overlay ownership,
  conservative defaults, capability failure policy, and retail-safe runtime
  smoke procedure. [`scripts/rtx_runtime_smoke.py`](../../scripts/rtx_runtime_smoke.py)
  keeps raster-module evidence separate from strict native-RT evidence.
...
- [`docs/fnql/UNDERWATER_VIEW.md`](./UNDERWATER_VIEW.md) documents the opt-in submerged-view compositor: the visual-only submersion feed and its `REF_API_VERSION` boundary, the shared wave/dispersion/absorption model, per-backend pass placement across the OpenGL-lineage, Vulkan, and RTX renderers, and the post-process limits.
...
meson compile -C .tmp\meson-dlight fnql_glx_x86 fnql_vk_x86
```
...
- `s_backend legacy` keeps the original software mixer/device backend available as a fallback path while implementing the retail QL voice and timing surface.
...
- Retail Steam receive audio uses `S_AddVoiceSamples`, not the raw/cinematic PCM stream. Both backends expose five identity-preserving remote-speaker lanes, apply `s_voiceVolume` independently, prebuffer with `s_voiceStep`, and refuse to steal a lane that still has queued audio. OpenAL reserves lanes above a protected gameplay-source floor; the legacy backend resamples from Steam's reported decoder rate into its DMA clock. `s_pvs` is an off-by-default, fail-open retail compatibility cull for positional world sounds, and `s_mixPreStep` restores the retail legacy-mixer cursor lead without removing the existing developer `s_mixOffset` override. Keep retail native cgame's `S_ClearLoopingSoundsFrame` import distinct from the older `S_ClearLoopingSounds(killall)` operation: every loop must be refreshed after a frame clear, while OpenAL must defer release until update so re-added loops remain sample-continuous.
...
- The OpenAL backend reads `sound/fnql-weapon-sounds.sndshd` as a small idTech4-style sound shader subset. Supported tuning keys include `minDistance`, `maxDistance`, `volume`, `volumeDb`, `shakes`, `reverb`, `wetLevel`, `frequencyShift`, and raw `sound/...` sample entries. The shipped `baseq3` file lives under `pkg/<game>/sound/`, is packed into `FnQL-pkg.fnz`, and imports FnQ3's weapon-sample tuning exactly apart from project-owned declaration names. A digest regression gate prevents its gain, falloff, pitch, shake, and wet-send values from drifting.
...
- `FNQL_AUDIO_LOOPBACK_TESTS` builds the deterministic audio test targets under [`tests/audio`](../../tests/audio). `fnql_audio_zone_tests` validates `.azb` runtime parsing, v1/v2 compatibility, zone priority selection, portal blend bounds, and invalid sidecar rejection; CTest registers it as `fnql_audio_zones`. `fnql_audio_recovery_tests` validates device-loss policy, `fnql_audio_tone_tests` validates source-class tone policy including the diegetic boundary and underwater shaping, `fnql_audio_voice_queue_tests` validates retail lane selection and bounded legacy resampling, and `fnql_audio_loopback_tests` dynamically loads OpenAL Soft to validate HRTF, distance and speaker routing, encoded/multichannel buffers, filters, EAXREVERB fallback, calibrated wet sends, and air absorption. The loopback test skips with exit code `77` when `ALC_SOFT_loopback` is unavailable.
...
- Per-voice tone classification and band shaping live in [`code/client/audio/shared/AudioTonePolicy.h`](../../code/client/audio/shared/AudioTonePolicy.h) so the policy can be exercised without an OpenAL device; `fnql_audio_tone_tests` is its regression gate. The class boundary that matters is diegetic versus non-diegetic: only `announcer`, `local`-only channels, and `sound/feedback/` samples are exempt from environment, medium, and occlusion shaping. Sounds carried by the listener's own entity are the `self` class: non-positional and dry like local audio, but still shaped by the medium the listener is in, so a first-person weapon loop is muffled under water like every other world sound. Do not fold `self` back into `local`.
...
- `FnQL-pkg.fnz` next to the executable has priority for packaged FnQL data-only sidecars. The loader tries game-dir-prefixed archive entries first, such as `baseq3/maps/foo.azb`, `baseq3/sound/fnql-weapon-sounds.sndshd`, `<gamedir>/maps/foo.azb`, or `<gamedir>/sound/fnql-weapon-sounds.sndshd`, then falls back to normal filesystem and pak search.
...
- The canonical stock Quake Live map inventory is the 149 `maps/*.bsp` members actually present in the legitimate retail Steam `baseq3/pak00.pk3`; it is recorded in `scripts/stock_ql_maps.py`. Menu metadata for optional and holiday Workshop maps is excluded when its BSP is absent. A matching `.azb` and conservative, opt-in `.fog` sidecar for every canonical map is tracked under `pkg/baseq3/maps/` and packed into `FnQL-pkg.fnz` as `baseq3/maps/` entries for installs/releases. Regenerate and audit the complete set from a local retail install with `python scripts/generate_stock_ql_sidecars.py --tool path/to/fnql-audiozonesc --prune path/to/QuakeLive`; the helper validates the retail inventory, extracts BSPs only into `.tmp/`, derives both sidecar types, records audit confidence and advisory warnings, checks exact coverage, and publishes only after a successful staged run. `--strict` is available for tuning work where every advisory should block publication; generated stock geometry commonly produces non-fatal overlap or sampled-coverage advisories. The standard weapon sound shader remains tracked under `pkg/baseq3/sound/` and is packed as `baseq3/sound/fnql-weapon-sounds.sndshd`.
