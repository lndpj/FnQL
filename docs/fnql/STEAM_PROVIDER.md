...
- A signed-in Steam account is validated in the same startup pass as the
  provider and Steam API libraries, because a present `steam_api` says nothing
  about whether the Steam client has an active user. A signed-out client session
  raises one startup notification offering proceed, retry, or quit. Proceeding
  keeps the same non-Steam fallback; retry re-runs provider startup so the user
  can sign in without relaunching. The notification is startup-only and
  client-only, so a dedicated server, a mid-session reconfigure, a build-script
  run, and any session without an interactive surface proceed unprompted.
...
- `com_steamLoginPrompt`: initialization policy, enabled by default; set `0` to
  continue silently instead of raising the startup sign-in notification.
...
- `cl_steamNameLock`: FnQL extension, enabled by default; set `0` to manage
  `name` locally instead of following the Steam persona.

## Local player identity

### Observed retail contract

Evidence in this section comes from the user's legitimate Steam installation of
`quakelive_steam.exe` `0.1.0.739`, the retail UI bundle inside its `web.pak`, and
the configs retail wrote for itself at
`<steamid>/baseq3/qzconfig.cfg` and `<steamid>/baseq3/repconfig.cfg`. Only files
headed `Generated by QUAKE LIVE` are treated as retail writes; the same install
also holds configs headed `Generated by FnQL`, which are FnQL's own output and
prove nothing about retail.

- The client registers `name` with the `UnnamedPlayer` default. That literal sits
  in the same executable string cluster as `country` and the recovered `cl_*`
  registration names.
- Retail persists no `name`. It appears in neither retail config, while the other
  userinfo keys do: `rate`, `model`, `headmodel`, `sex`, `color1`, `color2`, and
  `cg_predictItems` are all written to `repconfig.cfg`, and `cl_anonymous` to
  `qzconfig.cfg`. A value retail re-derives every session has nothing to persist.
- Retail does persist `seta country "GB"`, and it writes it to `repconfig.cfg`
  rather than `qzconfig.cfg`. That is the engine-replicated file, so `country` is
  an engine-managed publication of the Steam account country, not a preference.
- `PersonaStateChange_t` is a registered retail callback and the client publishes
  `users.persona.<id>.change`, so a profile rename is observed while running.
- The retail UI has no rename control. Its only identity reference is the
  read-only `qz_instance.playerName`, rendered through the bundle's
  `stripColors` helper (`/(\^[0-9])/g` then `/\^/g`), and the only cvars it ever
  writes are `password`, `net_port`, `bot_minplayers`, and `sv_serverType`.
  Renaming is a Steam profile action, not an in-game setting.

Taken together: retail registers a placeholder default, offers no way to change
it, never saves it, and holds a live persona subscription. What retail does with
a console `name` write is not claimed as recovered evidence -- the executable
carries no rename-rejection string, and this audit did not decompile the write
path.

### FnQL behavior

`name` keeps its retail default and its `CVAR_USERINFO` ownership, and `country`
is registered beside it as `CVAR_USERINFO | CVAR_ARCHIVE_ND | CVAR_PROTECTED`, so
FnQL's QL config writer places it in `repconfig.cfg` where retail put it. Whenever
the provider reports the identity capability, a non-zero local SteamID, and a
persona, the engine owns both keys: the persona is normalized into `name`, the ISO
3166-1 alpha-2 code into `country`, and a persona that normalizes away publishes
`UnnamedPlayer` instead of a local value.

One divergence is deliberate. FnQL keeps `name` archived where retail does not,
because FnQL supports provider-free sessions in which `name` really is a local
setting and would otherwise reset every launch. With a Steam identity the
archived value is overwritten before it can reach a server, so the difference is
confined to the lane retail does not have.

Normalization exists because a persona is arbitrary profile text while `name` is
an infostring value bounded by `MAX_NAME_LENGTH`:

- `\`, `"`, and `;` are removed. `Info_ValidateKeyValue` rejects them, so a
  persona containing one would drop the whole `name` key out of userinfo.
- Control bytes and DEL are removed; leading and trailing spaces are trimmed.
- Color escapes are preserved. Retail strips them when rendering, not at the
  identity source, so the published value stays a faithful copy of the persona.
- Truncation to the engine bound never splits a UTF-8 sequence and never leaves a
  trailing caret, which would otherwise consume the next character of any string
  the name is rendered into.

The identity is asserted before userinfo leaves the client, so a console or
module write that drifts from the persona is reverted ahead of both the reliable
userinfo update and the connection request. The reason is printed once per
distinct rejected value; adopting a new persona, an account change, or an
archived value is silent because none of those is a rename attempt. The provider
status is re-read on a bounded one-second interval, so a profile rename or a
sign-in transition is picked up without depending on a callback observer slot.

`cl_steamNameLock 0` is a documented FnQL extension for provider-free and mod
setups: it releases both keys and leaves `name` locally managed. The lock is
inert without a Steam identity, so provider-free sessions keep the inherited
...
That state reaches the user as a startup notification rather than as a later
connection failure. Both signed-out shapes are covered: a provider that reports
`FNQL_STEAM_RESULT_UNAVAILABLE` from startup, and a provider that starts
normally while `client_logged_on` or the local SteamID is absent. The engine
reads the account from the provider status only; it never fabricates a SteamID,
ticket, or successful sign-in to satisfy the check.

Retry releases the provider and repeats the whole load-and-start sequence, so
signing in to Steam and answering retry reaches the same state as a fresh
launch. Quit exits through the ordinary shutdown path. Proceeding is recorded in
`com_steamProviderDetail` and leaves the provider active for whatever it can
still do, which keeps the existing local-play identity check in `sv_client` as
the authoritative gate for a listen server. Platform notifications live behind
one `Sys_PromptStartupChoice` contract: a Win32 modal whose Continue, Try Again,
and Cancel buttons map to proceed, retry, and quit, and a terminal prompt on
Unix. Both report proceed when no interactive surface exists, so redirected
stdio, unattended hosts, and CI never block on a dialog.
