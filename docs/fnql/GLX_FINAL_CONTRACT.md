...
- Versioned C ABI: the engine-facing boundary is `REF_API_VERSION 14`,
...
| Version | `REF_API_VERSION` is bumped only when the whole engine ABI is deliberately versioned; the renderer-only liquid-interaction export moved it to `13` after the WebUI and QL host-font revisions, and the renderer-only submerged-view export moved it to `14`. |
