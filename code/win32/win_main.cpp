...
void *Sys_LoadLibrary( const char *name )
{
	...
	return reinterpret_cast<void *>( LoadLibrary( AtoW( name ) ) );
}
...
void *Sys_LoadFunction( void *handle, const char *name )
{
	...
	symbol = reinterpret_cast<void *>( GetProcAddress( static_cast<HMODULE>( handle ), name ) );
	if ( !symbol )
		dll_err_count++;
...
static void SetTimerResolution( void )
{
	...
	fnql::win::ScopedLibrary dll( LoadLibrary( T( "ntdll" ) ) );
	if ( dll )
	{
		pNtQueryTimerResolution = reinterpret_cast<pfnNtQueryTimerResolution>( GetProcAddress( dll.get(), "NtQueryTimerResolution" ) );
		pNtSetTimerResolution = reinterpret_cast<pfnNtSetTimerResolution>( GetProcAddress( dll.get(), "NtSetTimerResolution" ) );
