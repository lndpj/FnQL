...
static qboolean IsWindows7OrGreater( void ) {
	...
	rtl_get_version_f = reinterpret_cast<RtlGetVersionPtr>( GetProcAddress( ntdll, "RtlGetVersion" ) );
...
static fnql::win::ScopedComPtr<IMMDeviceEnumerator> pEnumerator;
static fnql::win::ScopedComPtr<IMMDevice> iMMDevice;
...
// Sound mixer thread
static DWORD WINAPI ThreadProc( LPVOID initEvent )
{
	...
	hInited = static_cast<HANDLE>( initEvent );
	...
	fnql::win::ScopedLibrary avrtLibrary( LoadLibraryW( L"avrt" ) );
	hAVRT = avrtLibrary.get();
	if ( hAVRT )
	{
		pAvSetMmThreadCharacteristicsW = reinterpret_cast<decltype( pAvSetMmThreadCharacteristicsW )>( GetProcAddress( hAVRT, "AvSetMmThreadCharacteristicsW" ) );
		pAvRevertMmThreadCharacteristics = reinterpret_cast<decltype( pAvRevertMmThreadCharacteristics )>( GetProcAddress( hAVRT, "AvRevertMmThreadCharacteristics" ) );
	...
	if ( iAudioRenderClient->lpVtbl->GetBuffer( iAudioRenderClient.get(), numFramesAvailable, &pData ) != S_OK )
	{
		Com_Printf( S_COLOR_YELLOW "WASAPI GetBuffer failed\n" );
		goto err_exit;
	}

	if ( iAudioRenderClient->lpVtbl->ReleaseBuffer( iAudioRenderClient.get(), numFramesAvailable, AUDCLNT_BUFFERFLAGS_SILENT ) != S_OK )
	{
	...
	if ( iAudioClient->lpVtbl->Start( iAudioClient.get() ) != S_OK )
	{
	...
	for ( ;; )
	{
		...
		hr = iAudioRenderClient->lpVtbl->GetBuffer( iAudioRenderClient.get(), numFramesAvailable, &pData );
		if ( hr == S_OK )
		{
			dwOffset = 0;
			...
			iAudioRenderClient->lpVtbl->ReleaseBuffer( iAudioRenderClient.get(), numFramesAvailable, 0 );
		}
	}

	iAudioClient->lpVtbl->Stop( iAudioClient.get() );
...
{
	(void)self;
	if ( DeviceID && wcscmp( DeviceID.get(), pwstrDeviceId ) == 0 )
	{
...
static qboolean SNDDMA_InitWASAPI( void )
{
	...
	fnql::win::ScopedCoTaskMem<WAVEFORMATEX> closest;
	DWORD					dwThreadID;
	fnql::win::ScopedHandle	hInited;
	qboolean				isfloat;
	HRESULT					hr;
	...
	DeviceID.reset(); // release old device id if exists
	iMMDevice->lpVtbl->GetId( iMMDevice.get(), DeviceID.receive() );
...
#if 0
	iAudioClient->lpVtbl->GetMixFormat( iAudioClient.get(), (WAVEFORMATEX**) &mixFormat );
	if ( mixFormat )
	{
	...
	hr = iAudioClient->lpVtbl->IsFormatSupported( iAudioClient.get(), AUDCLNT_SHAREMODE_SHARED, (const WAVEFORMATEX *) &desiredFormat, closest.receive() );
	if ( hr != S_OK )
	{
		if ( closest )
		{
			Com_Memcpy( &desiredFormat, reinterpret_cast<WAVEFORMATEXTENSIBLE *>( closest.get() ),
				closest.get()->wFormatTag == WAVE_FORMAT_EXTENSIBLE ? sizeof( WAVEFORMATEXTENSIBLE ) : sizeof( WAVEFORMATEX ) );
		}
		else
		{
	...
	if ( com_developer->integer )
	{
		...
		iAudioClient->lpVtbl->GetDevicePeriod( iAudioClient.get(), &defDuration, NULL );
	...
	hEvent.reset( CreateEvent( NULL, FALSE, FALSE, NULL ) );
	if ( !hEvent )
	{
	...
	if ( iAudioClient->lpVtbl->SetEventHandle( iAudioClient.get(), hEvent.get() ) != S_OK )
	{
		Com_Printf( S_COLOR_YELLOW "WASAPI: SetEventHandle() failed\n" );
		goto error5;
	}

	if ( iAudioClient->lpVtbl->GetService( iAudioClient.get(), IID_IAudioRenderClient,
		reinterpret_cast<void **>( iAudioRenderClient.receive() ) ) != S_OK )
	{
	...
	hInited.reset( CreateEvent( NULL, FALSE, FALSE, NULL ) );
	if ( !hInited )
	{
	...
	hThread.reset( CreateThread( NULL, 4096, ThreadProc, hInited.get(), 0, &dwThreadID ) );
	if ( !hThread )
	{
...
error2:
	...
	if ( pEnumerator && notification_client.lpVtbl->QueryInterface ) {
		pEnumerator->lpVtbl->UnregisterEndpointNotificationCallback( pEnumerator.get(), (IMMNotificationClient *)&notification_client );
	}
...
static fnql::win::ScopedComPtr<IDirectSound> pDS;
static fnql::win::ScopedComPtr<IDirectSoundBuffer> pDSBuf;
static fnql::win::ScopedComPtr<IDirectSoundBuffer> pDSPBuf;
static fnql::win::ScopedComInitialization s_soundCom;
...
void SNDDMA_Shutdown( void ) {
	...
	if ( pDS ) {
		...
		pDS->lpVtbl->SetCooperativeLevel( pDS.get(), g_wv.hWnd, DSSCL_PRIORITY );

		if ( pDSBuf ) {
			Com_DPrintf( "...stopping and releasing sound buffer\n" );
			pDSBuf->lpVtbl->Stop( pDSBuf.get() );
		}

		// only release primary buffer if it's not also the mixing buffer we just released
		const bool sharedPrimaryBuffer = pDSPBuf && pDSBuf.get() == pDSPBuf.get();
		pDSBuf.reset();
		if ( pDSPBuf && !sharedPrimaryBuffer ) {
			Com_DPrintf( "...releasing primary buffer\n" );
			pDSPBuf.reset();
		}
		else if ( sharedPrimaryBuffer ) {
			pDSPBuf.release();
		}
...
static qboolean SNDDMA_InitDS( void )
{
	...
	if( FAILED( hresult = CoCreateInstance(CLSID_DirectSound8, NULL, CLSCTX_INPROC_SERVER, IID_IDirectSound8,
	...
	if ( DS_OK != pDS->lpVtbl->SetCooperativeLevel( pDS.get(), g_wv.hWnd, DSSCL_PRIORITY ) )	{
		Com_Printf ("failed\n");
		SNDDMA_Shutdown();
		return qfalse;
	}
	...
	if (DS_OK == pDS->lpVtbl->CreateSoundBuffer(pDS.get(), &dsbuf, pDSBuf.receive(), NULL)) {
	...
	if ( DS_OK != pDSBuf->lpVtbl->Play(pDSBuf.get(), 0, 0, DSBPLAY_LOOPING) ) {
	...
	if ( DS_OK != pDSBuf->lpVtbl->GetCaps (pDSBuf.get(), &dsbcaps) ) {
...
void SNDDMA_BeginPainting( void ) {
	...
	if ( pDSBuf->lpVtbl->GetStatus (pDSBuf.get(), &dwStatus) != DS_OK ) {
		Com_Printf ("Couldn't get sound buffer status\n");
	}
	
	if (dwStatus & DSBSTATUS_BUFFERLOST)
		pDSBuf->lpVtbl->Restore (pDSBuf.get());
	
	if (!(dwStatus & DSBSTATUS_PLAYING))
		pDSBuf->lpVtbl->Play(pDSBuf.get(), 0, 0, DSBPLAY_LOOPING);

	// lock the dsound buffer
	reps = 0;
	dma.buffer = NULL;

	while ((hresult = pDSBuf->lpVtbl->Lock(pDSBuf.get(), 0, gSndBufSize, reinterpret_cast<LPVOID *>( &pbuf ), &locksize, 
...
void SNDDMA_Submit( void ) {
	...
	if ( pDSBuf ) {
		pDSBuf->lpVtbl->Unlock(pDSBuf.get(), dma.buffer, locksize, NULL, 0);
	}
}
