...
static int GLW_ChoosePFD( HDC hDC, PIXELFORMATDESCRIPTOR *pPFD )
{
	...
	pfds = static_cast<PIXELFORMATDESCRIPTOR *>( Z_Malloc( ( maxPFD + 1 ) * sizeof( PIXELFORMATDESCRIPTOR ) ) );
...
static qboolean GLW_InitOpenGLDriver( int colorbits )
{
	...
	if ( !glw_state.pixelFormatSet )
	{
		GLW_CreatePFD( &pfd, colorbits, depthbits, stencilbits, r_stereoEnabled->integer != 0 ? qtrue : qfalse );
		if ( ( tpfd = GLW_MakeContext( &pfd ) ) != TRY_PFD_SUCCESS )
		{
			...
			GLW_CreatePFD( &pfd, colorbits, depthbits, 0, r_stereoEnabled->integer != 0 ? qtrue : qfalse );
...
void UpdateMonitorInfo( const RECT *target ) 
{
	...
	if ( GetMonitorInfo( hMon, (LPMONITORINFO)&mInfo ) && EnumDisplaySettings( mInfo.szDevice, ENUM_CURRENT_SETTINGS, &devMode ) ) {
		...
		glw_state.desktopWidth = hDC ? GetDeviceCaps( hDC.get(), HORZRES ) : 0;
		glw_state.desktopHeight = hDC ? GetDeviceCaps( hDC.get(), VERTRES ) : 0;
		...
		else
		{
			...
			else
			{
				...
				for ( modeNum = 0 ; ; modeNum++ ) {
					...
					if ( devmode.dmPelsWidth >= static_cast<DWORD>( config->vidWidth )
						&& devmode.dmPelsHeight >= static_cast<DWORD>( config->vidHeight )
...
void GLimp_Init( glconfig_t *config )
{
	...
	// optional
#define GLE( ret, name, ... ) q##name = reinterpret_cast<decltype( q##name )>( GL_GetProcAddress( XSTRING( name ) ) )
	QGL_Swp_PROCS;
#undef GLE
