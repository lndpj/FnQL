...
qboolean R_RtShaderNativeSupported( const shader_t *materialShader )
{
	...
	/* The current RT vertex/material ABI carries only base UVs and one texture
	 * index.  Keep authored lightmaps raster-owned until closest-hit can sample
	 * their independent coordinates and texture without approximating them. */
...
qhandle_t RE_RegisterShader( const char *name ) {
	...
	/*
	 * Renderer-owned shaders use angle-bracket names so the client can request
	 * a prebuilt handle without accidentally creating an implicit image shader
	 * with the same name.
	 */
	if ( name[0] == '<' ) {
...
static void CreateInternalShaders( void ) {
	...
	/*
	 * FnQL's client-side player-highlight passes use these internal names.
	 * Their bespoke tess-end handlers provide the actual rim and stencil-shell
	 * rendering; these records preserve the established sort/blend contract
	 * and give the client stable renderer handles.
	 */
	InitShader( "<fnql_enemy_rim>", LIGHTMAP_NONE );
	stages[0].bundle[0].image[0] = tr.whiteImage;
	stages[0].bundle[0].tcGen = TCGEN_TEXTURE;
	stages[0].active = qtrue;
	...
	InitShader( "<fnql_enemy_outline>", LIGHTMAP_NONE );
	stages[0].bundle[0].image[0] = tr.whiteImage;
	stages[0].bundle[0].tcGen = TCGEN_TEXTURE;
	stages[0].active = qtrue;
...
static void CreateExternalShaders( void ) {
	...
	InitShader( "<fnql_lens_flare>", LIGHTMAP_NONE );
	stages[0].bundle[0].image[0] = lensFlareImage;
	stages[0].bundle[0].tcGen = TCGEN_TEXTURE;
	stages[0].active = qtrue;
	stages[0].bundle[0].rgbGen = CGEN_EXACT_VERTEX;
	stages[0].bundle[0].adjustColorsForFog = ACFF_NONE;
