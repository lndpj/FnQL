...
static void R_LoadGlobalFogForWorld( void )
{
	...
	/* Query and validate the sidecar size before allocation. A second check
	 * after the read closes the filesystem race without trusting a stale size. */
	Com_sprintf( filename, sizeof( filename ), "maps/%s.fog", s_worldData.baseName );
	expectedSize = ri.FS_ReadFile( filename, NULL );
	if ( expectedSize <= 0 ) {
		return;
	}
	if ( expectedSize > GLOBAL_FOG_SIDECAR_MAX_BYTES ) {
		ri.Printf( PRINT_WARNING,
			"WARNING: global fog sidecar %s is too large (%i bytes, limit %i)\n",
			filename, expectedSize, GLOBAL_FOG_SIDECAR_MAX_BYTES );
		return;
	}

	buffer.v = NULL;
	loadedSize = ri.FS_ReadFile( filename, &buffer.v );
	if ( !buffer.v || loadedSize <= 0 ) {
		if ( buffer.v ) {
	...
	if ( loadedSize != expectedSize ) {
		ri.Printf( PRINT_WARNING,
			"WARNING: global fog sidecar %s changed while loading (%i to %i bytes)\n",
			filename, expectedSize, loadedSize );
		ri.FS_FreeFile( buffer.v );
		return;
	}
	if ( loadedSize > GLOBAL_FOG_SIDECAR_MAX_BYTES ) {
		ri.Printf( PRINT_WARNING, "WARNING: global fog sidecar %s is too large (%i bytes, limit %i)\n",
			filename, loadedSize, GLOBAL_FOG_SIDECAR_MAX_BYTES );
		ri.FS_FreeFile( buffer.v );
		return;
	}
	if ( !R_GlobalFogParse( &s_worldData.globalFog, buffer.c, loadedSize,
		error, sizeof( error ) ) ) {
		R_GlobalFogClear( &s_worldData.globalFog );
...
#define STATIC_MAP_LIGHTS_SIDECAR_MAX_BYTES ( 256 * 1024 )
#define STATIC_MAP_LIGHTS_MAX_TOKENS 65536
#define STATIC_MAP_LIGHTS_MAX_DEPTH 32

static int s_staticMapLightsTokenCount;

static const char *R_StaticMapLightsNextToken( const char **p )
{
	if ( !p || !*p || s_staticMapLightsTokenCount >= STATIC_MAP_LIGHTS_MAX_TOKENS ) {
		return "";
	}
	s_staticMapLightsTokenCount++;
	return COM_ParseComplex( p, qtrue );
}

static qboolean R_StaticMapLightsSkipCompound( const char **p, const char *openToken )
{
	char closers[STATIC_MAP_LIGHTS_MAX_DEPTH];
	const char *token;
	int depth;

	if ( !openToken || ( openToken[0] != '{' && openToken[0] != '[' ) ) {
		return qfalse;
	}
	depth = 1;
	closers[0] = ( openToken[0] == '{' ) ? '}' : ']';

	while ( depth > 0 ) {
		token = R_StaticMapLightsNextToken( p );
		if ( !*token ) {
			return qfalse;
		}
		if ( !strcmp( token, "{" ) || !strcmp( token, "[" ) ) {
			if ( depth >= STATIC_MAP_LIGHTS_MAX_DEPTH ) {
				return qfalse;
			}
			closers[depth++] = ( token[0] == '{' ) ? '}' : ']';
		} else if ( !strcmp( token, "}" ) || !strcmp( token, "]" ) ) {
			if ( token[0] != closers[depth - 1] ) {
				return qfalse;
			}
			depth--;
		}
...
static qboolean R_StaticMapLightsParseFloat( const char **p, float *value )
{
	...
	if ( end == token || !end || *end != '\0' || errno == ERANGE ||
...
typedef enum {
	STATIC_LIGHT_FIELD_NAME = 1u << 0,
	STATIC_LIGHT_FIELD_TYPE = 1u << 1,
	STATIC_LIGHT_FIELD_ORIGIN = 1u << 2,
	STATIC_LIGHT_FIELD_DIRECTION = 1u << 3,
	STATIC_LIGHT_FIELD_COLOR = 1u << 4,
	STATIC_LIGHT_FIELD_INTENSITY = 1u << 5,
	STATIC_LIGHT_FIELD_RADIUS = 1u << 6,
	STATIC_LIGHT_FIELD_INNER_ANGLE = 1u << 7,
	STATIC_LIGHT_FIELD_OUTER_ANGLE = 1u << 8,
	STATIC_LIGHT_FIELD_CASTS_SHADOWS = 1u << 9,
	STATIC_LIGHT_FIELD_RESOLUTION = 1u << 10,
	STATIC_LIGHT_FIELD_PRIORITY = 1u << 11,
	STATIC_LIGHT_FIELD_STYLE = 1u << 12
} staticLightField_t;

static qboolean R_StaticMapLightsMarkField( uint32_t *seenFields, uint32_t field )
{
	if ( !seenFields || ( *seenFields & field ) ) {
		return qfalse;
	}
	*seenFields |= field;
	return qtrue;
	...
	*skipReason = 0;
	...
	while ( 1 ) {
		...
		if ( !Q_stricmp( key, "name" ) ) {
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_NAME ) ) {
				return qfalse;
			}
			token = R_StaticMapLightsNextToken( p );
			if ( !*token ) {
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_TYPE ) ) {
				return qfalse;
			}
			token = R_StaticMapLightsNextToken( p );
			if ( !*token ) {
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_ORIGIN ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_DIRECTION ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_COLOR ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_INTENSITY ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_RADIUS ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_INNER_ANGLE ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_OUTER_ANGLE ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_CASTS_SHADOWS ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_RESOLUTION ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_PRIORITY ) ) {
				return qfalse;
			}
			...
			if ( !R_StaticMapLightsMarkField( &seenFields, STATIC_LIGHT_FIELD_STYLE ) ) {
				return qfalse;
			}
...
static qboolean R_ParseStaticMapLights( const char *text )
{
	...
	qboolean hasLights = qfalse;

	s_staticMapLightsTokenCount = 0;
	p = text;
	token = R_StaticMapLightsNextToken( &p );
	if ( strcmp( token, "{" ) != 0 ) {
		return qfalse;
	...
	if ( !hasVersion || !hasLights || tr.staticMapLights.version != 1 ||
		s_staticMapLightsTokenCount >= STATIC_MAP_LIGHTS_MAX_TOKENS ) {
		return qfalse;
	}

	/* Only whitespace/comments may follow the root object. */
	token = R_StaticMapLightsNextToken( &p );
...
static void R_LoadStaticMapLightsForWorld( void )
{
	...
	if ( expectedSize > STATIC_MAP_LIGHTS_SIDECAR_MAX_BYTES ) {
		ri.Printf( PRINT_WARNING,
			"WARNING: static map lights sidecar %s is too large (%i bytes, limit %i)\n",
			filename, expectedSize, STATIC_MAP_LIGHTS_SIDECAR_MAX_BYTES );
		return;
	}
	...
	if ( loadedSize != expectedSize || loadedSize > STATIC_MAP_LIGHTS_SIDECAR_MAX_BYTES ) {
		ri.Printf( PRINT_WARNING,
			"WARNING: static map lights sidecar %s changed or exceeded its limit while loading (%i to %i bytes)\n",
			filename, expectedSize, loadedSize );
		ri.FS_FreeFile( buffer.v );
		return;
	}
	if ( memchr( buffer.c, '\0', (size_t)loadedSize ) != NULL ) {
		ri.Printf( PRINT_WARNING,
			"WARNING: static map lights sidecar %s contains an embedded NUL byte\n",
			filename );
		ri.FS_FreeFile( buffer.v );
		return;
	}
...
#define SURFACELIGHT_PROXY_SUBDIVIDE_MAX_BUCKETS \
	( SURFACELIGHT_PROXY_SUBDIVIDE_MAX_AXIS * SURFACELIGHT_PROXY_SUBDIVIDE_MAX_AXIS )
	...
	const vec3_t colorAccum, float area, vec3_t color )
{
	...
	/*
	 * RTX does not treat the raster lightmap as lighting authority.  BSP
	 * vertex color is only a final authored hue hint when the shader does not
	 * provide q3map_lightRGB/q3map_lightColor or q3map_lightImage.
	 */
	if ( area > 1.0f ) {
		VectorScale( colorAccum, 1.0f / area, vertexColor );
		if ( R_SurfaceLightColorIsUseful( vertexColor ) ) {
			color[0] = Com_Clamp( 0.0f, 1.0f, vertexColor[0] );
			color[1] = Com_Clamp( 0.0f, 1.0f, vertexColor[1] );
			color[2] = Com_Clamp( 0.0f, 1.0f, vertexColor[2] );
			return;
		}
	}
...
static qboolean R_SurfaceLightFloatIsFinite( float value )
{
	return ( value == value && value >= -FLT_MAX && value <= FLT_MAX ) ? qtrue : qfalse;
}

static qboolean R_SurfaceLightVec3IsFinite( const vec3_t value )
{
	return ( R_SurfaceLightFloatIsFinite( value[0] ) &&
		R_SurfaceLightFloatIsFinite( value[1] ) &&
		R_SurfaceLightFloatIsFinite( value[2] ) ) ? qtrue : qfalse;
}
	...
	vec3_t centroid, vec3_t cross, float *area )
{
	...
	if ( doubleArea <= 0.0f || !R_SurfaceLightFloatIsFinite( doubleArea ) ) {
		return qfalse;
	}
	...
	const surfaceLightProxyAccum_t *accum )
{
	...
	if ( !R_SurfaceLightFloatIsFinite( accum->area ) ||
		!R_SurfaceLightVec3IsFinite( accum->centroidAccum ) ||
		!R_SurfaceLightVec3IsFinite( accum->normalAccum ) ) {
	...
	if ( !R_SurfaceLightVec3IsFinite( origin ) ) {
		tr.surfaceLightProxies.skippedInvalid++;
		return qfalse;
	}
	...
	if ( !R_SurfaceLightFloatIsFinite( proxy->radius ) ||
		!R_SurfaceLightFloatIsFinite( proxy->footprintRadius ) ||
		!R_SurfaceLightFloatIsFinite( proxy->shadowCasterRadius ) ||
		!R_SurfaceLightFloatIsFinite( proxy->shadowConeAngle ) ) {
...
static qboolean R_SurfaceLightFinalizeSubdivision( surfaceLightProxySubdivision_t *subdiv )
{
	...
	for ( axis = 0; axis < 2; axis++ ) {
		...
		if ( !R_SurfaceLightFloatIsFinite( span ) ||
			!R_SurfaceLightFloatIsFinite( subdiv->targetSize ) || subdiv->targetSize <= 0.0f ) {
			return qfalse;
		}
	...
	const vec3_t centroid )
{
	...
	for ( axis = 0; axis < 2; axis++ ) {
		...
		if ( !R_SurfaceLightFloatIsFinite( coord ) ) {
			return -1;
		}
...
static qboolean R_BuildSurfaceLightFaceProxy( int surfaceIndex, const shader_t *shader,
...
static qboolean R_BuildSurfaceLightGridProxy( int surfaceIndex, const shader_t *shader,
...
static qboolean R_BuildSurfaceLightTriProxy( int surfaceIndex, const shader_t *shader,
	const srfTriangles_t *tri )
{
	...
	for ( i = 0; i + 2 < tri->numIndexes; i += 3 ) {
		...
		R_SurfaceLightAccumulateTriangle( tri->verts[i0].xyz, tri->verts[i1].xyz,
			tri->verts[i2].xyz, color0, color1, color2, &accum );
	}
	...
	if ( subdivide ) {
		...
		if ( subdivide ) {
			...
			for ( i = 0; i + 2 < tri->numIndexes; i += 3 ) {
				...
				R_SurfaceLightDrawVertColor( &tri->verts[i1], color1 );
				R_SurfaceLightDrawVertColor( &tri->verts[i2], color2 );
				R_SurfaceLightAccumulateTriangleForSubdivision( tri->verts[i0].xyz,
					tri->verts[i1].xyz, tri->verts[i2].xyz, color0, color1, color2,
					&subdiv, buckets );
			}
			bucketCount = subdiv.cells[0] * subdiv.cells[1];
			return R_AddSurfaceLightBucketedProxies( surfaceIndex, shader, &accum, buckets, bucketCount );
...
static void R_BuildSurfaceLightProxiesForWorld( void )
{
	...
	if ( r_surfaceLightProxyDebug && r_surfaceLightProxyDebug->integer ) {
		ri.Printf( PRINT_ALL,
			"RTX surfacelight proxies sources:%i built:%i point:%i linear:%i subdiv:%i/%i spatial:%i/%i skip sky:%i invalid:%i overflow:%i\n",
...
static const int lightmapFlags = IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION |
	IMGFLAG_LIGHTMAP | IMGFLAG_NOSCALE | IMGFLAG_COLORSPACE_LINEAR;
...
Quake Live adds a bounded advertisement-model lump after the normal IBSP
header. Invalid records are ignored individually so one damaged advert cannot
discard an otherwise valid retail map.
=================
*/
	...
	for ( i = 0; i < count; ++i ) {
		...
		if ( !QLBSP_AdvertisementGeometryFinite( in ) ) {
			ri.Printf( PRINT_DEVELOPER,
				"cell ID %d has non-finite advertisement geometry; ignoring it\n",
				LittleLong( in->cellId ) );
			continue;
		}

		bmodel = ( modelNum >= 0 && modelNum < s_worldData.numBmodels ) ?
			&s_worldData.bmodels[modelNum] : NULL;
		if ( !bmodel || bmodel->numSurfaces != 1 ) {
			ri.Printf( PRINT_DEVELOPER,
				"cell ID %d has no single-surface brush model; ignoring it\n",
				LittleLong( in->cellId ) );
			continue;
		}
		...
		VectorAdd( bmodel->bounds[0], bmodel->bounds[1],
			out[s_worldData.numAdvertisements].center );
		VectorScale( out[s_worldData.numAdvertisements].center, 0.5f,
			out[s_worldData.numAdvertisements].center );
		for ( j = 0; j < 3; ++j ) {
...
void RE_LoadWorldMap( const char *name ) {
...
#ifdef USE_VULKAN
	vk_rt_invalidate( "world_map_load" );
#endif

	R_ClearStaticMapLights();
	R_ClearSurfaceLightProxies();

	// Reset the authored world sun before used BSP sky shaders select it.
	R_ClearWorldSun();
	...
	if ( size >= (int)sizeof( dheader43_t ) ) {
		...
		if ( LittleLong( rawHeader->ident ) == BSP_IDENT &&
			LittleLong( rawHeader->version ) == BSP_VERSION_IHV ) {
			char translateError[128];

			if ( !BSP43_TranslateToV46( buffer.b, size, ri.Malloc, ri.Free,
				&translated, translateError, sizeof( translateError ) ) ) {
				ri.Error( ERR_DROP, "%s: %s could not translate IBSP v43: %s",
					__func__, name, translateError );
			}
			ri.FS_FreeFile( buffer.v );
	...
	if ( header->version != BSP_VERSION && header->version != BSP_VERSION_QL ) {
		ri.Error( ERR_DROP,
			"%s: %s has wrong version number (%i should be %i or %i)",
	...
	if ( header->version == BSP_VERSION_QL ) {
		const qlBspLumpResult_t lumpResult =
			QLBSP_ReadAdvertisementLump( buffer.b, (size_t)size,
				&qlAdvertisementsLump );
		if ( lumpResult == QL_BSP_LUMP_TRUNCATED_HEADER ) {
			ri.Error( ERR_DROP, "%s: %s has truncated QL extra lump header",
				__func__, name );
		}
		if ( lumpResult != QL_BSP_LUMP_OK ||
			!QLBSP_AdvertisementLumpShapeValid( &qlAdvertisementsLump ) ) {
			ri.Error( ERR_DROP,
				"%s: %s has wrong QL advertisement lump size/offset",
				__func__, name );
		}
	}
